...
from ..models.source_models import SourceInfo, SourceType, SourceFilter, SourceMetadata
...
class SourceService:
    ...
    async def register_crawl_result(
        ...
        try:
            for page in crawl_result.pages:
                source_info = await self._create_source_info(page, source_type, crawl_result)
                self._sources[source_info.id] = source_info
                registered_sources.append(source_info)
            
            logger.info(f"Registered {len(registered_sources)} sources from crawl result")
            return registered_sources
            
        except Exception as e:
            logger.error(f"Error registering crawl result: {e}")
            raise ToolError(f"Failed to register sources: {str(e)}")
    ...
    async def _create_source_info(
        ...
        metadata = SourceMetadata(
            ...
            file_size=page_content.metadata.get('content_length', len(page_content.content)),
    ...
    async def list_sources(
        self,
        filter_criteria: Optional[SourceFilter] = None
    ) -> List[SourceInfo]:
        """
        ...
        try:
            ...
            raise ToolError(f"Failed to list sources: {str(e)}")
    
    async def get_source(self, source_id: str) -> Optional[SourceInfo]:
        """
    ...
    async def update_source_stats(self, source_id: str) -> bool:
        ...
        try:
            ...
            if actual_chunk_count > 0:
                ...
                logger.debug(f"Updated source {source_id} with {actual_chunk_count} chunks")
                return True
    ...
    async def delete_source(self, source_id: str) -> bool:
        ...
        try:
            ...
            deleted_count = await self.vector_service.delete_documents_by_source(source_info.url)
    ...
    async def refresh_all_sources(self) -> Dict[str, int]:
        """
